...
namespace std
{
    struct StringBit
    {
        ...
        // Changes the value of the bit
        void operator=(char temp);

        // Returns if characters are equal
        bool operator==(const StringBit& other) const;

        // Returns if character and bit are equal
        bool operator==(char temp) const;

        // Returns if character and bit are not equal
        bool operator!=(char temp) const;

        // Returns if characters are not equal
        bool operator!=(const StringBit& other) const;

        operator char();

        // Inserts the content of StringBit in ostream
        friend std::ostream& operator<<(std::ostream& os, const StringBit& temp);
    };

    class ef_string {
        // Holds the bits
        std::vector<StringBit> m_bits{};
        ...
        void fill(const char* temp);

        using iterator = std::vector<StringBit>::iterator;

        using const_iterator = std::vector<StringBit>::const_iterator;

        using reverse_iterator = std::reverse_iterator<typename std::vector<StringBit>::iterator>;

        using const_reverse_iterator = std::reverse_iterator<typename std::vector<StringBit>::const_iterator>;
